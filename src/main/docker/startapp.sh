...
if [ "$1" = "inside" ]; then
    ...
    # read the mac address from $XDG_CONFIG_HOME/.qq_mac, if not exist, generate a random one
    if [ -f $XDG_CONFIG_HOME/.qq_mac ]; then
        qq_mac=$(cat $XDG_CONFIG_HOME/.qq_mac)
    else
        qq_mac=00\:$(hexdump -n5 -e '/1 ":%02X"' /dev/random | sed s/^://g)
        echo $qq_mac > $XDG_CONFIG_HOME/.qq_mac
    fi
